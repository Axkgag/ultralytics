...
def verify_image_label(args: tuple) -> list:
    ...
    elevator = len(args) > 8 and args[8]
    ...
    try:
        ...
        if os.path.isfile(lb_file):
            ...
            with open(lb_file, encoding="utf-8") as f:
                ...
                if not elevator and any(len(x) > 6 for x in lb) and (not keypoint):  # is segment
            ...
            if nl := len(lb):
                if keypoint:
                    ...
                    columns = 9 if elevator else 5
                    assert lb.shape[1] == columns, f"labels require {columns} columns, {lb.shape[1]} columns detected"
                    points = lb[:, 1:5]
                ...
                if elevator:
                    assert num_cls == 2, f"elevator labels require exactly 2 classes, got {num_cls}"
                    attrs = lb[:, 5:]
                    assert np.equal(attrs, attrs.astype(int)).all(), "elevator attributes must be integers"
                    slot1, slot2, light, light_valid = attrs.astype(int).T
                    assert ((0 <= slot1) & (slot1 <= 4)).all(), "slot1 labels must be in 0-4"
                    assert ((0 <= slot2) & (slot2 <= 10)).all(), "slot2 labels must be in 0-10"
                    assert np.isin(light, (0, 1)).all(), "light labels must be 0 or 1"
                    assert np.isin(light_valid, (0, 1)).all(), "light_valid labels must be 0 or 1"
                    floor = lb[:, 0].astype(int) == 0
                    legal = (
                        ((slot1 == 0) & (2 <= slot2) & (slot2 <= 10))
                        | ((slot1 == 1) & np.isin(slot2, (2, 3)))
                        | ((slot1 == 2) & (slot2 == 0))
                        | ((slot1 == 3) & (1 <= slot2) & (slot2 <= 10))
                        | ((slot1 == 4) & np.isin(slot2, (1, 2)))
                    )
                    assert (legal | ~floor).all(), "floor labels contain an unsupported slot1/slot2 combination"
                    assert (((slot1 == 0) & (slot2 == 0)) | floor).all(), "other labels require slot1=slot2=0"
                ...
                ne = 1  # label empty
                lb = np.zeros((0, (5 + nkpt * ndim) if keypoint else 9 if elevator else 5), dtype=np.float32)
        else:
            nm = 1  # label missing
            lb = np.zeros((0, (5 + nkpt * ndim) if keypoint else 9 if elevator else 5), dtype=np.float32)
        if keypoint:
